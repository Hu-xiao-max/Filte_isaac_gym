...
model_asset = gym.load_asset(sim, asset_root, "/home/pxing/code/PointNetGPD/PointNetGPD/data_000/ycb-tools/models/ycb/002_master_chef_can/google_512k/textured.urdf", asset_options)
...
scores[np.asarray(grasp_indice)[masks]]=-1.0#np.asarray将列表转换为数组
...
np.save('%s/npz_change/%03d_labels.npz'%(dataset_root, obj_idx),scores)
np.save('%s/npz_change/%03d_labels_point.npz'%(dataset_root, obj_idx),sampled_points)
