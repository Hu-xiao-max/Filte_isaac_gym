...
def main(nums):
    ...
    obj_idx = 5
    point_file_path='/home/tencent_go/Music/codes/multi_feature_get/dataset_process/npy/003_cracker_box.npy'
    sampled_points, offsets = get_model_grasps('%s/%03d_labels.npz'%(dataset_root, obj_idx),point_file_path)
    flag = False
        ...
        for v in view_inds:
            ...
            for a in angle_inds:
                ...
                for d in depth_inds:
                    ...
                    target_points.append(t)
                    depths.append(depth)
                    grasp_indice.append([point_ind,v,a])
                    num += 1
                    if num == nums:

                        flag = True
    return target_points,Rs,depths,grasp_indice


def run(target_points,Rs,depths,grasp_indice):
    ...
    #=================打印issac抓取成功的点=================================
    bad_grasp_ind=[]
    bad_grasp_set=np.asarray(grasp_indice)[masks]
    total_bad_num=bad_grasp_set.shape[0]
    for i in range(total_bad_num):
        bad_grasp_ind.append(bad_grasp_set[i,0])
    print('bad_grasp_ind:',bad_grasp_ind)

    print('bad_grasp_set:',bad_grasp_set)
    print('-----------------------------')
    print('bad_grasp_set shape:',bad_grasp_set.shape)
    print('+++++++++++++++++++++++++++++++')
...
if __name__=='__main__':
    nums=10
    target_points1,Rs1,depths,grasp_indice=main(nums)
    Rs,target_points=scripts()
    run(target_points,Rs,depths,grasp_indice)
