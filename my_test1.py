...
while not gym.query_viewer_has_closed(viewer):
    ...
    if initialize:
        ...
        if (model_dist>1e-4) & first_time:
            ...
            target_dist = torch.norm(to_target, dim=-1)#（1，36）
            print(target_dist)
            if torch.any(target_dist<0.001):
                ...
                if torch.all(target_dist<0.001):
                    ...
                    state=1#++++++++++++++++++++state==1是在这里赋值的++++++++++++++++++++++++++++
                    print('++++++++++++++++target_dist<0.001++++++++++++++++++++++++++')
        ...
        if(state==3):
            ...
            if torch.any(target_dist<1e-4):
                ...
                masks.extend(mask)#直接把mask加入masks的[]里
                grasped=False
                move_up=True                      
...
# print('++++++++++++++++++++++++',np.asarray(grasp_indice)[masks])
# print('________________________',grasp_indice)
...
print('++++++++++++++++++++++output++++++++++++++++++++++++++++')
data=open("/home/tencent_go/Music/codes/multi_feature_get/output.txt",'w+') 
print('这是个测试',bad_grasp_ind)
data.close()
...
scores[bad_grasp_set]=-1.0#np.asarray将列表转换为数组
